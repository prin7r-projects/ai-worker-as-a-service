...
**Onboarding (next wave).** Shiftledger's intake form asks for the help-desk URL, an inbound API token, and the resolution rule (Maya picks "ticket marked Closed by customer"). The first shift starts at 09:00 the next morning.
...
**Onboarding (next wave).** Devon contacts the Shiftledger team for a partner agreement — Shiftledger allows partner-priced unit rates and white-labeled receipts (the receipt PDF carries the agency's logo, not Shiftledger's, in the partner program).

**Recurring use.** Devon resells SDR shifts and CS shifts to his clients at a 30 % markup. Each client gets a monthly receipt, white-labeled. Devon settles with Shiftledger via the same NOWPayments rail.

**Exit.** Devon can churn out at any time; partner credits are honored on a 30-day refund basis for unused outcome receipts.

## Journey 3 — "Procurement evaluates Shiftledger as a vendor"
...
**Discovery.** Procurement is filling a vendor-evaluation matrix for "AI ops automation". Shiftledger appears alongside Manus, Lindy, Devin, Cognition. Shiftledger is the only entry in the matrix with a per-outcome unit price.
...
**Onboarding (next wave).** Shiftledger's enterprise track does a one-call security review (data flow diagram from this doc, tax + invoice handling under NOWPayments, EU hosting option). Contract = annual purchase order with a fixed pool of outcome receipts.
...
**Exit.** The pool's unused balance refunds at the contracted unit rate; Shiftledger never holds customer cash beyond cleared receipts.
...
- Verification is a feature, not a footnote. Each persona evaluates Shiftledger against the question: *can I trust the line item?* Doc 04 (pain points) and the landing's verification block are the answers.
